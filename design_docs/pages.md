...
- 数値データ
    - 現在進行中のタスク数（開始日が今日以前かつstatusがactiveのタスクが対象）
    - 今までの累計タスク完了数（statusがcompletedのタスクが対象）
    - 今月のタスク完了数（statusがcompletedかつ最終更新日時が今月のタスクが対象）
    - 今月の作業時間（作業実績のstart_atが今月のレコードが対象で、work_timeを）
    - 今までの累計作業時間（作業実績のstart_atが今月のレコードが対象）
    - 今までの累計サブタスク完了数（v2）
    - 今月のサブタスク完了数（v2）
- スケジュールの遅れを可視化
